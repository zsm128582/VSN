...
from RMT import RetBlock , RetNetRelPos2d
##########################################################################
## Layer Norm
...
class uLayer(nn.Module):
    def __init__(self,
        ...
        if chunkwise_recurrent:
            ...
            flag = 'whole'
        self.Relpos = RetNetRelPos2d(embed_dim=embed_dim , num_heads=num_heads,heads_range=head_range,initial_value=1e-6)
        self.blocks = nn.ModuleList(
            [
                RetBlock(flag ,embed_dim,num_heads,ffn_dim,drop_path,layerscale,layer_init_values)
...
class Utentive(nn.Module):
    def __init__(
        ...
        level = 0
        # 这些需要bhwc的输入
        self.encoder_level1 = uLayer(dim * 2**level,
                                      num_heads=heads[level],
                                      head_range=heads_ranges[level],
                                      block_nums=num_blocks[level],
                                      ffn_dim= dim * ffn_expansion_factor[level],
                                      chunkwise_recurrent=chunkwise_recurrents[level],
                                      )
        ...
        level = 1
        self.encoder_level2 = uLayer(dim * 2**level,
                                num_heads=heads[level],
                                head_range=heads_ranges[level],
                                block_nums=num_blocks[level],
                                ffn_dim= dim * ffn_expansion_factor[level],
        ...
        level = 2
        self.encoder_level3 = uLayer(dim * 2**level,
                        num_heads=heads[level],
                        head_range=heads_ranges[level],
                        block_nums=num_blocks[level],
                        ffn_dim= dim * ffn_expansion_factor[level],
        ...
        level = 3
        self.latent = uLayer(dim * 2**level,
                        num_heads=heads[level],
                        head_range=heads_ranges[level],
                        block_nums=num_blocks[level],
                        ffn_dim= dim * ffn_expansion_factor[level],
        ...
        self.decoder_level3 = uLayer(dim * 2**level,
                        num_heads=heads[level],
                        head_range=heads_ranges[level],
                        block_nums=num_blocks[level],
                        ffn_dim= dim * ffn_expansion_factor[level],
        ...
        level = 1
        self.decoder_level2 = uLayer(dim * 2**level,
                        num_heads=heads[level],
                        head_range=heads_ranges[level],
                        block_nums=num_blocks[level],
                        ffn_dim= dim * ffn_expansion_factor[level],
        ...
        level = 0
        # 最后一层没有reduce 所以需要 * 2
        self.decoder_level1 = uLayer(dim * 2,
                        num_heads=heads[level],
                        head_range=heads_ranges[level],
                        block_nums=num_blocks[level],
                        ffn_dim= dim * ffn_expansion_factor[level],
